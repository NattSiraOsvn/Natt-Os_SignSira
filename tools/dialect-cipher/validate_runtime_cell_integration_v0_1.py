...
BRIDGE_PATH = Path("src/cells/kernel/khai-cell/domain/services/language_spine_runtime_bridge.py")
OUT_PATH = Path("audit/validators/runtime_cell_integration_validator_report_v0_1.json")
...
def main() -> None:
    LanguageSpineRuntimeBridge = load_bridge()
    bridge = LanguageSpineRuntimeBridge()
    report = bridge.process_fixture_file()
    for row in report["results"]:
        ...
        if row["frame"]["decode_key"] != row["adapter_validation"]["decode_key"]:
            raise SystemExit("FAIL: decode_key mismatch for " + row["origin_text"])
    ...
    print("RUNTIME_CELL_INTEGRATION_PASS")
