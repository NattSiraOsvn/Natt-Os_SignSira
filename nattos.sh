...
for root, dirs, files in os.walk(src):
    ...
    for f in files:
        ...
        for ln, line in enumerate(lines, 1):
            ...
            # Điều 7
            if dia7_pat.search(line) and "//TODO" not in line and "// FIX" not in line:
                violations.append({
                    "dieu": "Điều 7",
                    "severity": "🔴 CRITICAL",
                    "cell": cell,
            ...
            # Điều 9
            if dia9_pat.search(line) and "nattos-server" not in path and "nattos-sheets" not in path:
                violations.append({
                    "dieu": "Điều 9",
                    "severity": "🟡 WARN",
                    "cell": cell,
...
history = history[-30:]  # Giữ 30 lần chạy cuối
os.makedirs(".nattos-twin", exist_ok=True)
