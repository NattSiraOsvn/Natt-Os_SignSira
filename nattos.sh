...
# ═══════════════════════════════════════════════════════════════
# S32 — ROOT SCAN (Vật thể lạ ở root)
# ═══════════════════════════════════════════════════════════════
hdr "32" "ROOT SCAN — Vật thể lạ & Cấu trúc repo"

# Files lạ ở root (không phải thư mục chuẩn)
KNOWN_ROOT=("src" "nattos-server" "packages" "clients" "extensions" "docs" "node_modules" ".git" ".nattos-twin")
KNOWN_FILES=("tsconfig.json" "package.json" "nattos.sh" ".gitignore" "Dockerfile" ".dockerignore" "README.md" "natt-os-twin.json")
...
while IFS= read -r f; do
  ...
  # Bỏ qua known files
  is_known=false
  for kf in "${KNOWN_FILES[@]}"; do [[ "$fname" == "$kf" ]] && is_known=true && break; done
  # Cảnh báo file lạ
  $is_known || FOREIGN_FILES+=("$fname")
done < <(find . -maxdepth 1 -type f ! -name ".*" 2>/dev/null | sort)

if [[ ${#FOREIGN_FILES[@]} -eq 0 ]]; then
  ok "Root files: CLEAN — không có vật thể lạ"; inc_ok
else
  warn "Vật thể lạ ở root (${#FOREIGN_FILES[@]} files) — chưa vào đúng chỗ:"
  for ff in "${FOREIGN_FILES[@]}"; do
    echo "    🔍  $ff"
  done
  inc_warn "ROOT: ${#FOREIGN_FILES[@]} foreign files at repo root"
fi

# Kiểm tra thư mục chuẩn có tồn tại không
for d in clients extensions docs; do
  if [[ -d "$d" ]]; then
    FILE_COUNT=$(find "$d" -type f 2>/dev/null | wc -l | tr -dc '0-9')
    ok "$d/: EXISTS ($FILE_COUNT files)"; inc_ok
  else
    info "$d/: chưa có — sẽ tạo khi cần"
  fi
done

# ═══════════════════════════════════════════════════════════════
# S33 — CLIENTS & EXTENSIONS (Apple Ecosystem Model)
# ═══════════════════════════════════════════════════════════════
hdr "33" "CLIENTS & EXTENSIONS — Ecosystem Health"

# clients/
if [[ -d "clients" ]]; then
  echo -e "  ${W}33a. Clients:${N}"
  CLIENT_TOTAL=0
  for client_dir in clients/*/; do
    [[ -d "$client_dir" ]] || continue
    cname=$(basename "$client_dir")
    cfiles=$(find "$client_dir" -type f 2>/dev/null | wc -l | tr -dc '0-9')
    HAS_PKG=$([[ -f "$client_dir/package.json" ]] && echo "✅" || echo "⚠️ ")
    HAS_SRC=$([[ -d "$client_dir/src" ]] && echo "✅" || echo "⚠️ ")
    echo "    $HAS_PKG $cname | $cfiles files | pkg:$HAS_PKG src:$HAS_SRC"
    ((CLIENT_TOTAL++)) || true
  done
  [[ "$CLIENT_TOTAL" -eq 0 ]] && info "clients/: folder trống" || ok "$CLIENT_TOTAL client apps found"; inc_ok
else
  info "clients/: chưa có — Vision Engine chưa được đặt vào đây"
  inc_warn "CLIENTS: folder không tồn tại — Vision Engine cần vào clients/natt-vision/"
fi

# extensions/
if [[ -d "extensions" ]]; then
  echo -e "  ${W}33b. Extensions:${N}"
  EXT_TOTAL=0
  for ext_dir in extensions/*/; do
    [[ -d "$ext_dir" ]] || continue
    ename=$(basename "$ext_dir")
    efiles=$(find "$ext_dir" -type f 2>/dev/null | wc -l | tr -dc '0-9')
    SWIFT_COUNT=$(find "$ext_dir" -name "*.swift" 2>/dev/null | wc -l | tr -dc '0-9')
    echo "    📦  $ename | $efiles files | Swift: $SWIFT_COUNT"
    ((EXT_TOTAL++)) || true
  done
  [[ "$EXT_TOTAL" -eq 0 ]] && info "extensions/: folder trống — Kim SwiftUI chưa vào" || ok "$EXT_TOTAL extensions found"; inc_ok
else
  info "extensions/: chưa có — Kim SwiftUI sẽ vào đây"
fi

# ═══════════════════════════════════════════════════════════════
# S34 — DOCS & SPECS
# ═══════════════════════════════════════════════════════════════
hdr "34" "DOCS & SPECS — Knowledge Base"

if [[ -d "docs" ]]; then
  SPEC_COUNT=$(find docs -name "*.md" 2>/dev/null | wc -l | tr -dc '0-9')
  ok "docs/: EXISTS | $SPEC_COUNT .md files"; inc_ok

  echo -e "  ${W}Spec files:${N}"
  while IFS= read -r spec; do
    fname=$(basename "$spec")
    fsize=$(wc -l < "$spec" 2>/dev/null | tr -dc '0-9')
    # Kiểm tra age - file mới trong 7 ngày
    if find "$spec" -mtime -7 -type f 2>/dev/null | grep -q .; then
      echo "    🆕  $fname ($fsize lines) — UPDATED recently"
    else
      echo "    📄  $fname ($fsize lines)"
    fi
  done < <(find docs -name "*.md" 2>/dev/null | sort)

  # SCAR check
  SCAR_COUNT=$(grep -r "FS-0" docs/ 2>/dev/null | grep -oE "FS-[0-9]+" | sort -u | wc -l | tr -dc '0-9')
  if [[ "$SCAR_COUNT" -gt 0 ]]; then
    LATEST_SCAR=$(grep -r "FS-0" docs/ 2>/dev/null | grep -oE "FS-[0-9]+" | sort -u | tail -1)
    ok "SCARs documented: $SCAR_COUNT | Latest: $LATEST_SCAR"; inc_ok
  else
    warn "Không tìm thấy SCAR records trong docs/"; inc_warn "DOCS: SCAR registry không có trong docs/"
  fi
else
  warn "docs/: MISSING — chưa có knowledge base"
  inc_warn "DOCS: folder không tồn tại"
fi

# Check SCAR trong bangmf
BANGMF=$(find src/governance/memory/bang -name "bangmf_*.json" 2>/dev/null | sort | tail -1)
if [[ -n "$BANGMF" ]]; then
  SCAR_IN_MF=$(python3 -c "import json; d=json.load(open('$BANGMF')); scars=d.get('scars',d.get('active_scars',[])); print(len(scars) if isinstance(scars,list) else len(scars.keys()))" 2>/dev/null || echo "?")
  ok "bangmf SCARs active: $SCAR_IN_MF"; inc_ok
fi

# ═══════════════════════════════════════════════════════════════
# S35 — SOVEREIGN & SENSITIVE FILES
# ═══════════════════════════════════════════════════════════════
hdr "35" "SOVEREIGN & SENSITIVE FILES"

# .sovereign folder
SOVEREIGN_DIR="$HOME/.sovereign"
if [[ -d "$SOVEREIGN_DIR" ]]; then
  SOV_COUNT=$(ls "$SOVEREIGN_DIR" 2>/dev/null | wc -l | tr -dc '0-9')
  ok ".sovereign/: EXISTS — $SOV_COUNT files (ngoài repo, an toàn)"; inc_ok
  for expected in fsp.png ssp.png tsp.png lsp.png cccd_front.png cccd_back.png; do
    if [[ -f "$SOVEREIGN_DIR/$expected" ]]; then
      echo "    ✅  $expected"
    else
      echo "    ❌  $expected — MISSING"
      inc_warn "SOVEREIGN: $expected missing"
    fi
  done
else
  warn ".sovereign/: MISSING — SiraSign keys chưa được lưu"
  inc_warn "SOVEREIGN: ~/.sovereign không tồn tại"
fi

# gitignore check cho sensitive patterns
if [[ -f ".gitignore" ]]; then
  GITIGNORE_SOVEREIGN=$(grep -c "\.sovereign\|sirasign\*\|nattos-google-sa" .gitignore 2>/dev/null || echo 0)
  if [[ "$GITIGNORE_SOVEREIGN" -ge 2 ]]; then
    ok ".gitignore: sensitive patterns protected ($GITIGNORE_SOVEREIGN rules)"; inc_ok
  else
    warn ".gitignore: thiếu protection cho sovereign/sirasign keys"
    inc_warn "GITIGNORE: sensitive files không được protect đủ"
  fi
fi

# Phát hiện file sensitive nằm sai chỗ (trong repo)
LEAKED=$(find . -maxdepth 5 \( -name "*.sovereign" -o -name "sirasign*.json" -o -name "*cccd*" -o -name "*nattos-google-sa*" \) ! -path "./.git/*" 2>/dev/null)
if [[ -n "$LEAKED" ]]; then
  fail "SENSITIVE FILES LỌT VÀO REPO:"
  echo "$LEAKED" | sed 's/^/    🚨  /'
  inc_fail "SECURITY: sensitive files detected in repo"
else
  ok "Không phát hiện sensitive files trong repo"; inc_ok
fi
...
for ev in sorted(orphan): print(f"      ⚠️  orphan: '{ev}' from {producers[ev][0]}")
for ev in sorted(dead)[:5]: print(f"      💀  dead_sub: '{ev}' (subscriber nhưng không ai emit)")
...
# List all engines by cell
by_cell={}
for eng,path in declared.items():
    parts=path.replace("\\","/").split("/")
    cell="core"
    for i,x in enumerate(parts):
        if x in("business","kernel","infrastructure") and i+1<len(parts): cell=parts[i+1]; break
    by_cell.setdefault(cell,[]).append(eng)
print(f"\n  Engine inventory per cell:")
for cell in sorted(by_cell):
    wired_here=[e for e in by_cell[cell] if e in alive]
    dead_here=[e for e in by_cell[cell] if e in dead]
    status="✅" if not dead_here else "⚠️ "
    print(f"    {status} {cell}: {', '.join(sorted(by_cell[cell]))}")
os.makedirs(".nattos-twin",exist_ok=True)
