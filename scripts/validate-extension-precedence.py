...
Patch Rule 4: variable-aware bypass detection.
Previous v0.1 miss case `VALIDATOR="path/to/file.ts"` + `npx tsx "$VALIDATOR"`
vì regex match literal, không resolve shell variable.

v0.2 approach: 2-pass scan.
  Pass 1: extract `VAR="path/to/file.ts"` assignments
  Pass 2: scan `tsx|node|npx tsx <path-or-$VAR>`, resolve $VAR via Pass 1 map

SCAR-20260423-SCANNER-VARIABLE-BLIND vaccin applied.

Other rules unchanged from v0.1.

Drafter: Băng · session 20260423
Binding: SPEC_HOST_FIRST_RUNTIME_v1.0 §5 #9 + Kim SPEC_EXTENSION_PRECEDENCE v1.0
Commit path designed: scripts/validate-extension-precedence.py (replace v0.1)
"""
...
def extract_shell_var_assignments(content):
    """
    Pass 1: extract shell variable assignments pointing to .ts files.

    Patterns handled:
      VAR="path/to/file.ts"
      VAR='path/to/file.ts'
      VAR=path/to/file.ts
      export VAR="path/to/file.ts"

    Returns dict {var_name: ts_path}.
    """
    assignments = {}
    patterns = [
    ...
    for pat in patterns:
        for m in pat.finditer(content):
            var_name = m.group(1)
            ts_path = m.group(2)
            assignments[var_name] = ts_path
    return assignments


def check_ts_bypass_canonical_v2(ts_path, all_scripts_content):
    """
    Pass 2: variable-aware bypass check.
    Modes reported: 'direct-literal' or 'variable-resolved ($VAR)'.
    """
    bypasses = []
...
def main():
    ...
    if bypass_count == 0:
        ...
        print(f"  FAIL: {bypass_count} bypass call(s) — patch route qua canonical")
