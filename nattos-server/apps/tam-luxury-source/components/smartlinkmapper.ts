...
const SmartLinkMapper: React.FC<SmartLinkMapperProps> = ({ currentRole, logAction }) => {
  ...
  const [filter, setFilter] = useState<'ALL' | 'DRAFT' | 'LINKED' | 'POSTED'>('ALL');
  ...
  const handlePostEntry = (id: string) => {
    /* Fix: using journalId property of AccountingEntry */
                        ...
                        <tbody>
                           {entry.entries.map((line, idx) => (
                              <tr key={idx} className="border-b border-white/5">
                                 <td className="py-4">
                                    <span className="text-white font-bold">{line.accountCode}</span>
