...
export interface WarrantyConfidenceFactor {
  name:   string;
  weight: number; // 0-1
  score:  number; // 0-100
}

export interface WarrantyConfidenceScore {
  cellId:    'warranty-cell';
  total:     number; // 0-100 weighted average
  factors:   WarrantyConfidenceFactor[];
  timestamp: string;
  status:    'HEALTHY' | 'DEGRADED' | 'CRITICAL';
}

export function calculateWarrantyConfidence(params: {
  hasActiveData:   boolean;
  lastAuditPassed: boolean;
  engineHealthy:   boolean;
}): WarrantyConfidenceScore {
  const factors: WarrantyConfidenceFactor[] = [
