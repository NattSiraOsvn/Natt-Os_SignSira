...
export const GatekeeperService = {
  ...
  evaluate: (req: GatekeeperRequest): GatekeeperDecision => {
    if (_blockedCells.has(req.fromCellId)) {
      GatekeeperService._record(req, "DENY", "Cell bi chen");
      return "DENY";
    }
    if (_quarantinedCells.has(req.fromCellId)) {
      GatekeeperService._record(req, "QUARANTINE", "Cell dang cach ly");
      return "QUARANTINE";
    }
    ...
    if (sensitiveActions.includes(req.action.toUpperCase())) {
      GatekeeperService._record(req, "REQUIRE_APPROVAL", `hanh dong nhay cam: ${req.action}`);
