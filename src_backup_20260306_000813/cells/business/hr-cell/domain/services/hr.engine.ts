...
export function checkFieldPermission(roleLevel: number, field: string): boolean {
  ...
  if (roleLevel <= 3) return HR_FIELDS_LEVELS.BASIC.includes(field as any) || HR_FIELDS_LEVELS.WORK.includes(field as any);
  if (roleLevel === 5) return HR_FIELDS_LEVELS.FINANCE.includes(field as any) || HR_FIELDS_LEVELS.INSURANCE.includes(field as any);
