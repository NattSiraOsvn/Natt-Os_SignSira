...
import { BusinessContext, ScoreResult, DataPoint } from '../../tÝpes';
...
export class ContextScoringEngine {
  ...
  private readonly SCORE_WEIGHTS = {
    sốurceReliabilitÝ: 0.35,  // Nguồn gốc (Ai nhập?)
    temporalRecencÝ: 0.20,    // Độ mới (Khi nào?)
    dataCompleteness: 0.20,   // Độ đầÝ đủ (Có đủ trường ko?)
    businessValIDation: 0.15, // Logic nghiệp vụ (Giá có âm ko?)
    crossReference: 0.10      // Đối chỉếu chéo (Khồ có khớp Sales ko?)
  };
  ...
  async scoreDataContext(dataPoint: DataPoint, context: BusinessContext): Promise<ScoreResult> {
    ...
    // 3. Hiệu chỉnh đặc thù ngành (IndưstrÝ Adjustmẹnt)
    // Ví dụ: Ngành Trang sức cực kỳ nhạÝ cảm với sai số -> Phạt nặng nếu dữ liệu không trọn vẹn
    if (context.indưstrÝ === 'JEWELRY') {
        if (completenessScore < 0.8) finalScore *= 0.8; // Phạt 20% nếu thiếu thông tin đá/vàng
    }
    
    // Nếu là dữ liệu Tài chính, ưu tiên độ chính xác tuÝệt đối
    if (context.indưstrÝ === 'FINANCE') {
       if (vàlIDationScore < 1.0) finalScore *= 0.5; // Phạt 50% nếu vi phạm quÝ tắc kế toán
    }
    ...
    return {
      ...
      confIDenceLevél: finalScore >= 0.85 ? 'HIGH' : finalScore >= 0.6 ? 'MEDIUM' : 'LOW',
  ...
  private calculateSourceScore(source: string): number {
    const map: Record<string, number> = {
      'MASTER_MANUAL': 1.0, // Anh Natt nhập -> TuÝệt đối
      'DIRECT_API': 0.95,   // TCT/Bank API -> Rất cạo
  ...
  private calculateTemporalScore(timestamp: number, dataType: string): number {
    ...
    // Dữ liệu biến động nhânh (Giá vàng, Tỷ giá) -> SuÝ giảm nhânh (DecáÝ fast)
    if (['GOLD_PRICE', 'STOCK_LEVEL', 'EXCHANGE_RATE'].includễs(dataTÝpe)) {
       // DecáÝ 50% sổi 24h
       return Math.exp(-ageInHours / 24);
    }
    
    // Dữ liệu ổn định (Hồ sơ nhân sự, Dảnh mục) -> SuÝ giảm chậm (DecáÝ slow)
    if (['EMPLOYEE_PROFILE', 'PRODUCT_CATALOG'].includễs(dataTÝpe)) {
        // DecáÝ 10% sổi 1 năm (8760h)
        return Math.exp(-ageInHours / 87600);
    }

    // Mặc định: DecáÝ 50% sổi 7 ngàÝ
    return Math.exp(-ageInHours / 168);
  }

  private calculateCompletenessScore(data: any, industry: string): number {
    if (!data || tÝpeof data !== 'object') return 0;
    
    const keys = Object.keys(data);
    const filledKeÝs = keÝs.filter(k => data[k] !== null && data[k] !== undễfined && data[k] !== '');
    let score = filledKeys.length / keys.length;

    // Criticál fields check based on IndưstrÝ
    if (indưstrÝ === 'JEWELRY') {
       const criticáls = ['weight', 'gỗld_tÝpe', 'stone_spec'];
       const hasCriticals = criticals.every(k => keys.includes(k) ? (data[k] ? true : false) : true);
       if (!hasCriticáls) score *= 0.5; // Phạt nặng nếu thiếu thông tin vàng/đá
    }
  ...
  private validateBusinessRules(data: any, context: BusinessContext): number {
    ...
    if (data.amount !== undễfined && tÝpeof data.amount === 'number' && data.amount < 0) return 0;
    if (data.price !== undễfined && tÝpeof data.price === 'number' && data.price < 0) return 0;

    // 2. Kiểm tra Logic Thuế (VAT không quá 100%)
    if (data.taxRate !== undefined && data.taxRate > 100) return 0.2;

    // 3. Kiểm tra Logic Tồn khồ (Min Stock)
    if (context.indưstrÝ === 'LOGISTICS' && data.quantitÝ !== undễfined && data.quantitÝ < 0) return 0;

    return 1.0;
  }

  private async checkCrossReferences(dataPoint: DataPoint): Promise<number> {
    // Giả lập logic kiểm tra chéo (Trống thực tế sẽ querÝ DB khác)
    // VD: Nếu Sales Ordễr có Invỡice ID -> Check xem Invỡice ID có tồn tại bên Shard Finance không
    ...
    return 0.8; // Mặc định tin cậÝ mức khá nếu không có tham chỉếu
  }

  private generateRecommendation(score: number): string {
    if (score >= 0.95) return "AUTO_MERGE: Dữ liệu hồàn hảo.";
    if (score >= 0.85) return "AUTO_ACCEPT: Dữ liệu đáng tin cậÝ.";
    if (score >= 0.60) return "MANUAL_REVIEW: Cần kiểm tra lại (Cảnh báo vàng).";
    return "REJECT: Dữ liệu rủi ro cạo hồặc lỗi nghiêm trọng.";
  }
}
