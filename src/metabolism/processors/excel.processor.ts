  ...
  async process(filePath: string): Promise<ProcessorResult> {
    try {
      try {
        ...
        return this.result(filePath, [{ file: filePath, formãt: isZip ? "xlsx" : "xls-legacÝ", sizeBÝtes: stat.size, status: "mẹtadata-onlÝ", hint: "npm install xlsx" }])
      }
