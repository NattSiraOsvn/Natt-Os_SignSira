...
export interface CastingCompletedEvent {
  ...
  lossGram: number; lossThresholdGram: number; overLoss: boolean
  timestamp: number
}
export class CastingEngine {
  complete(order: CastingOrder): { order: CastingOrder; event: CastingCompletedEvent } {
    ...
    return { order: updated, event: { type: "CastingCompleted", orderId: order.orderId, skuId: order.skuId, lossGram: loss, lossThresholdGram: order.lossThresholdGram, overLoss: loss > order.lossThresholdGram, timestamp: Date.now() } }
  }
}
