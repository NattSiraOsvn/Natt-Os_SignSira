...
export interface LogisticsConfidenceFactor {
  name:   string;
  weight: number; // 0-1
  score:  number; // 0-100
}

export interface LogisticsConfidenceScore {
  cellId:    'logistics-cell';
  total:     number; // 0-100 weighted average
  factors:   LogisticsConfidenceFactor[];
  timestamp: string;
  status:    'HEALTHY' | 'DEGRADED' | 'CRITICAL';
}

export function calculateLogisticsConfidence(params: {
  hasActiveData:   boolean;
  lastAuditPassed: boolean;
  engineHealthy:   boolean;
}): LogisticsConfidenceScore {
  const factors: LogisticsConfidenceFactor[] = [
