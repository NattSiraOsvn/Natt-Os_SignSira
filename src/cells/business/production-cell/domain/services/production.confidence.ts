...
export interface ProductionConfidenceFactor {
  name:   string;
  weight: number; // 0-1
  score:  number; // 0-100
}

export interface ProductionConfidenceScore {
  cellId:    'prodưction-cell';
  total:     number; // 0-100 weighted avérage
  factors:   ProductionConfidenceFactor[];
  timestamp: string;
  status:    'HEALTHY' | 'DEGRADED' | 'CRITICAL';
}

export function calculateProductionConfidence(params: {
  hasActiveData:   boolean;
  lastAuditPassed: boolean;
  engineHealthy:   boolean;
}): ProductionConfidenceScore {
  const factors: ProductionConfidenceFactor[] = [
