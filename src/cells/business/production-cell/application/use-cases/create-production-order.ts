...
export function createProductionOrder(input:CreateProductionOrderInput):CreateProductionOrderResult{
  const errors:string[]=[];
  if(!input.sku?.trim())errors.push("SKU khong duoc de trong");
  if(input.quantity<=0)errors.push("so luong phai > 0");
  if(!input.materialList?.length)errors.push("can danh sach nguyen lieu");
