...
export function reportMaterialLoss(input:ReportMaterialLossInput):ReportMaterialLossResult{
  if(input.actualQuantity>input.expectedQuantity)return{success:false,lossAmount:0,lossRate:0,isAlert:false,error:"luong thuc te khong the lon hon ke hoach"};
