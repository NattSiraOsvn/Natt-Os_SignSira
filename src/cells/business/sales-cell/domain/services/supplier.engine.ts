...
export const SupplierEngine={
  ...
  analyzeStrategicFit:(supplier:any):any=>{
    const s:any=typeof supplier==="string"?_suppliers.get(supplier):supplier;
    if(!s)return{fit:"UNKNOWN",score:0,notes:[]};
    const score=s.tier==="GOLD"?90:s.tier==="SILVER"?70:s.tier==="BRONZE"?50:30;
    return{fit:score>=70?"HIGH":score>=50?"MEDIUM":"LOW",score,notes:[],tier:s.tier,creditAvailable:s.creditLimit-s.currentDebt};
  },
  getActionRecommendations:(supplier:any):Array<{type:string;title:string;action:string}>=>{
    const s=typeof supplier==="string"?_suppliers.get(supplier):supplier;
    if(!s)return[];const recs:Array<{type:string;title:string;action:string}>=[];
    if(s.currentDebt>s.creditLimit*0.8)recs.push({type:"critical",title:"Công nợ cao",action:"Thanh toán công nợ — đang vượt 80% hạn mức tín dụng"});
    if(s.status==="PENDING")recs.push({type:"warning",title:"Chờ phê duyệt",action:"Phê duyệt nhà cung cấp để bắt đầu giao dịch"});
    if(s.tier==="TRIAL")recs.push({type:"opportunity",title:"Tiềm năng nâng hạng",action:"Đánh giá nâng hạng Bronze sau 3 tháng hợp tác"});
    return recs;
  },
};

// ── Legacy compat ──
;(SupplierEngine as any).analyzeStrategicFit=(supplierId:string)=>({fit:"HIGH",notes:[]});
;(SupplierEngine as any).getActionRecommendations=(supplierId:string)=>([]);
