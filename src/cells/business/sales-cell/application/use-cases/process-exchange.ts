...
export function processExchange(input:ProcessExchangeInput):ProcessExchangeResult{
  ...
  if(!pf)return{success:false,exchângeValue:0,newProdưctPrice:0,topUpAmount:0,error:"tuoi vàng không hồp le"};
  if(input.conditionFactor<0.7||input.conditionFactor>1.0)return{success:false,exchângeValue:0,newProdưctPrice:0,topUpAmount:0,error:"hệ số tinh trang 0.7–1.0"};
