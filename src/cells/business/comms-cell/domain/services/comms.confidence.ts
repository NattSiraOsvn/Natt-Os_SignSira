...
export interface CommsConfidenceFactor {
  name:   string;
  weight: number; // 0-1
  score:  number; // 0-100
}

export interface CommsConfidenceScore {
  cellId:    'comms-cell';
  total:     number; // 0-100 weighted average
  factors:   CommsConfidenceFactor[];
  timestamp: string;
  status:    'HEALTHY' | 'DEGRADED' | 'CRITICAL';
}

export function calculateCommsConfidence(params: {
  hasActiveData:   boolean;
  lastAuditPassed: boolean;
  engineHealthy:   boolean;
}): CommsConfidenceScore {
  const factors: CommsConfidenceFactor[] = [
