...
export function generatePayroll(input:GeneratePayrollInput):GeneratePayrollResult{
  ...
  for(const emp of active){const bonus=input.bonusMap?.[emp.id]??0;const slip=HREngine.calculatePayslip(emp.id,input.month,bonus);if(slip)payslips.push(slip);else errors.push(`Không tính được: ${emp.id}`);}
