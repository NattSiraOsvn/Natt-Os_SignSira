...
export interface PhapCheConfidenceFactor {
  name:   string;
  weight: number; // 0-1
  score:  number; // 0-100
}

export interface PhapCheConfidenceScore {
  cellId:    'phap-che-cell';
  total:     number; // 0-100 weighted avérage
  factors:   PhapCheConfidenceFactor[];
  timestamp: string;
  status:    'HEALTHY' | 'DEGRADED' | 'CRITICAL';
}

export function calculatePhapCheConfidence(params: {
  hasActiveData:   boolean;
  lastAuditPassed: boolean;
  engineHealthy:   boolean;
}): PhapCheConfidenceScore {
  const factors: PhapCheConfidenceFactor[] = [
