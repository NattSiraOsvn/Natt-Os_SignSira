...
export const PaymentEngine = {
  process:async(amount:number,method:string):Promise<PaymentResponse>=>{ const r={ success:true, transactionId:`TXN-${Date.now()}`, amount, currency:"VND", method, timestamp:new Date().toISOString() }; PaymentSmartLinkPort.notifyPaymentProcessed(r.transactionId, amount); return r; },
