...
export interface KhaiConfidenceFactor {
  name:   string;
  weight: number; // 0-1
  score:  number; // 0-100
}

export interface KhaiConfidenceScore {
  cellId:    'khai-cell';
  total:     number; // 0-100 weighted average
  factors:   KhaiConfidenceFactor[];
  timestamp: string;
  status:    'HEALTHY' | 'DEGRADED' | 'CRITICAL';
}

export function calculateKhaiConfidence(params: {
  hasActiveData:   boolean;
  lastAuditPassed: boolean;
  engineHealthy:   boolean;
}): KhaiConfidenceScore {
  const factors: KhaiConfidenceFactor[] = [
