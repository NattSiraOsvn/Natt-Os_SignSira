...
export const SUPER_DICTIONARY = deepFreeze({
  // ... (Phần meta và document_definitions)
  meta: {
  ...
  conflict_resolution_rules: {
    SALES: {
      ...
      threshold: 0.2, // Chênh lệch confidence > 20% thì auto-resolve
    ...
    FINANCE: {
      ...
      threshold: 0.05, // Tài chính rất nhạy cảm -> threshold thấp để dễ kích hoạt Manual Review
...
class SuperDictionaryClass {
  private static instance: SuperDictionaryClass;
  private dictionary: Map<string, BusinessTerm> = new Map();
  private version: string = SUPER_DICTIONARY.meta.version;
  ...
  static getInstance(): SuperDictionaryClass {
    if (!SuperDictionaryClass.instance) {
      SuperDictionaryClass.instance = new SuperDictionaryClass();
    }
    return SuperDictionaryClass.instance;
  }
  ...
  private ingestLegacyData(): void {
    ...
    Object.entries(SUPER_DICTIONARY.business_units).forEach(([key, val]) => {
      this.addTerm({
        code: `UNIT_${key}`,
        name: `Khối ${key}`,
...
export default SuperDictionaryClass.getInstance();
